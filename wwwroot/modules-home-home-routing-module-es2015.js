...
/* harmony import */ var _project_components_create_project_dialog_createProjectDialog_component__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../../../project/components/create-project-dialog/createProjectDialog.component */ "./src/modules/project/components/create-project-dialog/createProjectDialog.component.ts");
/* harmony import */ var src_modules_project_services__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! src/modules/project/services */ "./src/modules/project/services/index.ts");
/* harmony import */ var src_app_shared_services_loader_service__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! src/app/shared/services/loader.service */ "./src/app/shared/services/loader.service.ts");
/** @format */
...
let HomeComponent = class HomeComponent {
    constructor(_auth, _dataService, _dialog, _projectService, _loaderServ) {
        this._auth = _auth;
...
let DataResolve = class DataResolve {
    constructor(_dataService, _auth) {
        this._dataService = _dataService;
        this._auth = _auth;
    }
    resolve(route) {
...
/* harmony import */ var _components__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ../../components */ "./src/modules/project/components/index.ts");
/** @format */
...
let ProjectComponent = class ProjectComponent {
    //#endregion
    constructor(_dialog, _taskService, _projectService) {
        this._dialog = _dialog;
    ...
    ngDoCheck() {
        this.dataSource.data.forEach(x => {
            if (_models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Onway === x.status &&
                new Date(x.deadline).getTime() < new Date().getTime()) {
                x.status = _models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Expired;
            }
            else if (_models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Expired === x.status &&
                new Date(x.deadline).getTime() > new Date().getTime()) {
    ...
    _setStatus(tasks) {
        tasks.forEach(checkingTask => {
            if (new Date(checkingTask.deadline).getTime() < new Date().getTime() &&
                ...
                this._taskService
                    .updateStatus$(checkingTask.id, _models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Expired)
                    .subscribe(() => { }, error => console.error(error.message));
                checkingTask.status = _models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Expired;
            }
            else if (new Date(checkingTask.deadline).getTime() > new Date().getTime() &&
                checkingTask.status === _models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Expired) {
                this._taskService
                    .updateStatus$(checkingTask.id, _models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Onway)
                    .subscribe(() => { }, error => console.error(error.message));
            }
        });
    }
    ...
    _updateTask(task) {
        this.dataSource.data = this.dataSource.data.map(x => x.id === task.id ? task : x);
    }
    updateProjectDialog() {
    ...
    dropTable(event) {
        ...
        this._taskService.updatePrioraty$(this.dataSource.data).subscribe(() => { }, error => console.error(error.message));
    }
    setDone(element) {
        this._taskService.updateStatus$(element.id, _models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Done).subscribe(() => { }, error => console.error(error.message));
        element.status = _models_task_models__WEBPACK_IMPORTED_MODULE_6__["Status"].Done;
    }
    removeTask(id) {
        this.dataSource.data = this.dataSource.data.filter(x => x.id !== id);
        this._taskService.removeTask$(id).subscribe(() => { }, error => console.error(error.message));
    }
    removeProject() {
        this.isDeleted = true;
        this._projectService.removeProject$(this.projectId).subscribe(() => { }, error => console.error(error.message));
    }
};
...
], ProjectComponent.prototype, "matSort", null);
Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
...
ProjectModule = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["NgModule"])({
        imports: [
            ...
            _angular_forms__WEBPACK_IMPORTED_MODULE_4__["ReactiveFormsModule"],
            _angular_forms__WEBPACK_IMPORTED_MODULE_4__["FormsModule"],
            src_app_app_module__WEBPACK_IMPORTED_MODULE_9__["MaterialModule"],
        ...
        providers: [
            ...
            ..._guards__WEBPACK_IMPORTED_MODULE_7__["guards"]
        ],
        declarations: [
...
let ProjectService = class ProjectService {
    ...
    removeProject$(id) {
        return this._http.delete(`${this.baseUrl}/${this._auth.UserId}/remove/${id}`, {
            responseType: 'json'
        });
    }
    updateProject$(id, name) {
        return this._http.put(`${this.baseUrl}/${this._auth.UserId}/update`, { id, name }, {
            responseType: 'json'
        });
    }
    createProject$(name) {
        return this._http.post(`${this.baseUrl}/${this._auth.UserId}/create`, { name }, {
            responseType: 'json'
        });
    }
};
...
let TaskService = class TaskService {
    ...
    createTask$(projectId, name, deadline, priority) {
        return this._http.post(`${this.baseUrl}/${this.userId}/create/${projectId}`, { name, deadline, priority }, {
            responseType: 'json'
        });
    }
    updateTask$(id, name, deadline) {
        return this._http.put(`${this.baseUrl}/${this.userId}/update`, { id, name, deadline }, {
            responseType: 'json'
        });
    }
    updateStatus$(id, status) {
        return this._http.put(`${this.baseUrl}/${this.userId}/updateStatus`, { id, status }, {
            responseType: 'json'
        });
    }
    updatePrioraty$(tasks) {
        return this._http.put(`${this.baseUrl}/${this.userId}/updatePrioraty`, { tasks }, {
            responseType: 'json'
        });
    }
    removeTask$(id) {
